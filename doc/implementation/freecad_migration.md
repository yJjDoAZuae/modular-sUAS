...
| --- | --- | --- | --- | --- |
| IP-FC-1 | todo | Measure `freecadcmd` startup and single-part build time; decide subprocess-per-part vs long-lived worker | — | [freecad_migration.md §OQ-ARCH-3](../architecture/freecad_migration.md) |
| IP-FC-2 | todo | Build `params_snapshot.py` — capture and diff the `Parameters`/`NoseParameters` tree for every variant, replacing the exact tier `scad_snapshot.py` provides today | — | [freecad_migration.md §Verification](../architecture/freecad_migration.md) |
...
| IP-FC-4 | todo | Export the nose and tail OML as STEP from the committed `.vsp3`; replace `oml/vsp_*.stl` and repoint `oml_ref()` | — | [freecad_migration.md §UC-9](../architecture/freecad_migration.md) |
| IP-FC-5 | todo | Prototype the corner in FreeCAD using `Part::` primitives and booleans; compare volume and bounding box against its OpenSCAD reference | — | [corner.md](../design/corner.md), [freecad_migration.md §OQ-ARCH-1](../architecture/freecad_migration.md) |
...
| IP-FC-16 | todo | Write the cowl interior-surface algorithm document — per-layer 2D inset, and the rule that supplies material at near-horizontal surfaces | IP-FC-7 | [cowl.md §6.2](../design/cowl.md) |
...
| IP-FC-31 | todo | Resolve OQ-DES-CW7: establish whether the committed `.vsp3` is current, and make the OML export reproducible from it rather than hand-driven | — | [cowl.md §OQ-DES-CW7](../design/cowl.md) |
...
| IP-FC-19 | blocked (OQ-ARCH-6, IP-FC-17) | UC-4 — assemblies with joints for unit, nose, tail and full fuselage | IP-FC-17, IP-FC-18 | [freecad_migration.md §OQ-ARCH-6](../architecture/freecad_migration.md) |
...
| IP-FC-21 | blocked (OQ-ARCH-7, IP-FC-14) | UC-7a — part drawings: dimension scheme, template, TechDraw pages | IP-FC-14 | [freecad_migration.md §OQ-ARCH-7](../architecture/freecad_migration.md) |
