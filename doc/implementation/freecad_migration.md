...
| IP-FC-43 | todo | Remove the buttress cut's factor of two by folding it into the value. All four cutting modules extrude `height = 2*buttress_thickness` with the parameter at 0.05, so the cut is 0.1 mm; the parameter becomes `buttress_cut_thickness = 0.1` and the `2*` comes out of `top_buttress`, `top_diag_buttress`, `side_buttress` and `bottom_buttress`. **The extruded height is unchanged, so no geometry moves** and the flown parts stay reproducible. Decided by OQ-DES-CW8 2026-08-18 — no longer blocked. **Do it in the same pass as IP-FC-29**, which renames the same call sites; verify by diffing the generated `.scad` and confirming the only changes are the identifier and the dropped factor, every extruded height still 0.1 mm | IP-FC-29 | [cowl.md §OQ-DES-CW8](../design/cowl.md) |
