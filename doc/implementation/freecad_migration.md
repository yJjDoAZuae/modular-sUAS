...
| IP-FC-70 | todo | **`render_variant.py` cannot select the FreeCAD backend, so the supported way to build one part produces no `.FCStd`.** It never calls `set_backend`, so it always renders through OpenSCAD whatever the caller wants. That makes it unusable for the thing the migration exists to produce, and it is the tool the project directs people to instead of `-D` overrides on a driver — so the gap is in the one-part path everyone is told to use. Add a `--backend` argument matching `fuselage_variants.py`'s. Related housekeeping in the same tools: `compare_backends.py` still defaults its scratch tree to `Path(OUTPUT_DIR).parent / 'compare_backends'`, i.e. inside `src/Fuselage`, which is why a crashed run left 2.6 GB in the source tree; it is gitignored and `--scratch` overrides it, but the default belongs outside the repo | IP-FC-14 | |
