...
| IP-FC-80 | todo | **Inventory every difference against the frozen baseline, and record them as a ledger.** OQ-ARCH-15 decided 2026-08-18 that `variant_output_baseline` does not move for the rest of the port, so each departure from it must be enumerated and justified rather than absorbed. The three known so far come from a five-part sample — `corner_FX_0.5` and `bulkhead_end_anchor` from `299633c`, `boom_bulkhead_center_single` from `e11d04a` — and there are 576 parts, so this needs **one full comparison run**, which is expensive and should be scheduled deliberately. Each ledger entry carries the part, the measured difference, the commit, and why it is correct. The comparison tools then read the ledger: green when the only differences are accepted ones, red the moment a new one appears. Until this exists `verify_sweep_change.py` cannot gate corner, bulkhead or boom bulkhead work | IP-FC-83 | [freecad_migration.md §OQ-ARCH-15](../architecture/freecad_migration.md) |
| IP-FC-81 | todo | **Record what the frozen baseline actually is.** `variant_output_baseline` is not a render of this repository — its stored `.stl.scad` files name their library under the pre-migration `Archive\Alex\Designs\modular_sUAS\Fuselage\` tree — and no tree in the corpus records the commit that produced it. Write a manifest beside it giving provenance, date and tool versions. Freezing a reference makes this more important rather than less: a ledger of departures means nothing without a statement of what they depart from. Not a drift gate — that is IP-FC-80's job | — | [freecad_migration.md §OQ-ARCH-15](../architecture/freecad_migration.md) |
| IP-FC-82 | todo | **Bring `mesh_stats.same_geometry` to the project's own tolerance rule.** Volume compared relatively rather than against an absolute 1e-6 mm³, bounding box against the `U`-scaled tolerance OQ-ARCH-12 already defines, triangle count reported but not disqualifying. The constants exist in `compare_backends.bbox_tol()` and `check_unread_rows.VOLUME_TOL`; this finishes OQ-ARCH-12 rather than deciding anything new. Measured motivation: the boom bulkhead at 22,680 against 20,472 triangles with volumes agreeing to 8×10⁻⁷ relative is reported as changed geometry on triangle count alone. Decided by OQ-ARCH-16 2026-08-18 | — | [freecad_migration.md §OQ-ARCH-16](../architecture/freecad_migration.md) |
| IP-FC-83 | todo | **Add a sampled surface-distance comparison.** Volume and bounding box are proxies and can agree while a surface has moved; a distance in millimeters is the only measure that can be checked against what a printer holds. Sample points from each mesh and measure each against the other surface, reporting maximum and mean — **not every vertex**, so cost is set by sample size rather than mesh size, which matters at ~90,000 triangles per cowl across 576 parts. Runs alongside IP-FC-82's criteria: the cheap ones screen, the distance adjudicates. Sample size, how points are drawn, and the `U`-scaled threshold are open to implementation and should be measured, not guessed — a sparse sample makes the result a lower bound and can miss a small displaced feature. Decided by OQ-ARCH-16 2026-08-18 | IP-FC-82 | [freecad_migration.md §OQ-ARCH-16](../architecture/freecad_migration.md) |
